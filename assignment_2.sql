...
SELECT last_name as 'Last Name', department_id as 'Department Number'
FROM employees
WHERE employee_id = 176;

-- 3. display the last name and salary for all employees whose salary is not in the range of $5,000 and $12,000.
 
SELECT last_name  as 'Last Name', salary  as 'Salary'
FROM employees
WHERE salary NOT BETWEEN 5000 AND 12000 ;

-- 4. Display the employee last name, job ID, and start date of employees hired between February 20, 1998, and May 1, 2002. Order the query in ascending order by start date.

SELECT last_name  as 'Last Name', job_id as 'Job ID', hire_date as 'Start Date'
FROM employees
WHERE hire_date BETWEEN '1998-02-20' AND '2002-06-01' 
...
-- 6. list the last name and salary of employees who earn between $5,000 and $12,000, and are in department 20 or 50. Label the columns Employee and Monthly Salary, respectively. 
...
SELECT last_name  as 'Last Name', hire_date as 'Hare Date'
FROM employees
...
SELECT last_name  as 'Last Name', job_id as 'Job ID'
FROM employees
WHERE manager_id IS NULL;
--
SELECT employees.last_name as 'Last Name', jobs.job_title as 'Job Title'
FROM employees
...
SELECT employees.last_name as 'Last Name', employees.salary  as 'Salary', jobs.max_salary - employees.salary as 'Commission'
FROM employees
INNER JOIN jobs ON employees.job_id = jobs.job_id
WHERE jobs.max_salary - employees.salary > 0.00
ORDER BY Commission DESC, employees.salary DESC;

-- Alias can be only use in OREDER By, not in Wheere or Having
...
SELECT employees.last_name as 'Last Name', jobs.job_title as 'Job Title', employees.salary  as 'Salary'
FROM employees
INNER JOIN jobs ON employees.job_id = jobs.job_id
WHERE employees.salary NOT IN ('2500','3500','7000') 
AND jobs.job_title IN ('Stock Clerk','Sales Representative');
...
FROM employees
INNER JOIN jobs  ON employees.job_id = jobs.job_id  
WHERE (jobs.max_salary - employees.salary) = ( employees.salary  * 0.2);


-- QUESTION RELATED TO JOINING

-- 1 Display the country names in region  id 2.
...
-- 2 Display the region name of region id 2.

SELECT regions.region_name as 'Region Name'
FROM regions
-- INNER JOIN regions  ON countries.region_id = regions.region_id
WHERE regions.region_id=2;

-- 3 Display all the office addresses those are in US.

SELECT regions.region_name as 'Region Name', countries.country_name as 'Country Name'
FROM countries
INNER JOIN regions  ON countries.region_id = regions.region_id
WHERE countries.country_id='US';

-- 4 Display the department names which are in location 1700.
